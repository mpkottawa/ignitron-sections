...
void SparkDataControl::updateBatteryLevel() {
    ...
    // Convert ADC reading to pin voltage (0–3.3 V range)
    float analogVoltage = (analogReading / BATTERY_MAX_LEVEL) * 3.3f;

    // Scale back up to pack voltage
    float dividerGain = (VOLTAGE_DIVIDER_R1 + VOLTAGE_DIVIDER_R2) / VOLTAGE_DIVIDER_R2;

    // Optional fine-tune calibration (default = 1.0, increase slightly if Pack reads low vs DMM)
    const float BAT_CALIB = 1.00f; 
    float batteryVoltage = analogVoltage * dividerGain * BAT_CALIB;

#elif BATTERY_TYPE == BATTERY_TYPE_AMP
    float batteryVoltage = SparkStatus::getInstance().ampBatteryLevel();
#endif

    // Set battery level according to thresholds
